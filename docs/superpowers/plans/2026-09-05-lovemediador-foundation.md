# LOVEMEDIADOR Foundation — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Bootstrap the LOVEMEDIADOR backend (Fastify + Postgres + Prisma) with Auth (register/login/2FA), Couples (invite/accept), and LGPD consent — the foundation on which the AI Core will run.
...
**Spec:** `docs/superpowers/specs/2026-09-05-lovemediador-design.md`
...
Append:
```
DATABASE_URL=postgresql://love:love@localhost:5432/lovemediador?schema=public
```
...
export async function request2fa(userId: string): Promise<{ sent: true }> {
  ...
  await getSmsSender().send(user.phone, `LOVEMEDIADOR: seu código é ${code}. Válido por 5 minutos.`);
  return { sent: true };
}
